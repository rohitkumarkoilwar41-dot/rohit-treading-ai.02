...
def fetch_binance_klines(symbol, interval, limit=1000):
    try:
        # Map streamlit tf to binance interval
...
def load_chart_fixed(ticker, tf):
    # === CRYPTO KE LIYE BINANCE SE DIRECT - GAP KHATAM ===
    crypto_map = {
        "BTC-USD": "BTCUSDT",
        "ETH-USD": "ETHUSDT", 
        "SOL-USD": "SOLUSDT",
    ...
    if ticker in crypto_map:
        ...
        if not df.empty and len(df)>20:
            ...
            return df
    # === GOLD/SILVER & FOREX KE LIYE YAHOO ===
...
if st.session_state.tracker_on and st.session_state.tracker_pair:
    ...
    if live_result:
        ...
        if abs(buy-sell) <= 2:
            tg_status = "HOLD " + str(buy) + "% / " + str(sell) + "%"
            tg_emoji = "HOLD"
        elif buy > sell:
            tg_status = "BUY " + str(buy) + "% | SELL " + str(sell) + "%"
            tg_emoji = "BUY"
        else:
            tg_status = "SELL " + str(sell) + "% | BUY " + str(buy) + "%"
            tg_emoji = "SELL"
