...
if st.button("START ANALYSIS"):
    ...
    # === NEW: Liquidity Sweep (EQH/EQL Stop Hunt) ===
    liq_bull=False; liq_bear=False; eqh_level=None; eql_level=None
    # Last 30 candles me Equal Highs / Equal Lows dhundo
    look_liq = 30
    highs_30 = h.iloc[-look_liq-5:-5]  # 5 candle pehle tak, taaki live se alag ho
    lows_30 = l.iloc[-look_liq-5:-5]
    # EQH: 2 se zyada highs jo 0.15 ATR ke andar ho
    tol = atr_v * 0.15
    for i in range(len(highs_30)-1):
        for j in range(i+1, len(highs_30)):
            if abs(float(highs_30.iloc[i]) - float(highs_30.iloc[j])) < tol:
                ...
                # Kya price ne is EQH ko sweep kiya? high > EQH fir close < EQH
                if h.iloc[-2] > eqh_level and c.iloc[-1] < eqh_level and last < eqh_level:
                    liq_bear = True  # Bearish sweep - upar stop hunt karke neeche aaya
                break
        ...
        for j in range(i+1, len(lows_30)):
            if abs(float(lows_30.iloc[i]) - float(lows_30.iloc[j])) < tol:
                ...
                if l.iloc[-2] < eql_level and c.iloc[-1] > eql_level and last > eql_level:
                    liq_bull = True  # Bullish sweep - neeche stop hunt karke upar aaya
                break
    ...
    # FINAL FIX: Pro SMC - 50% Mitigation + No LIVE inside + 0.5 ATR min
    ...
    if bull_ob: score+=15; reasons.append(f"Bullish OB {bull_ob[0]:.2f}-{bull_ob[1]:.2f}")
    if bear_ob: score-=15; reasons.append(f"Bearish OB {bear_ob[0]:.2f}-{bear_ob[1]:.2f}")
    if liq_bull: score+=35; reasons.append(f"Liquidity Sweep Bullish - EQL {eql_level:.2f} Sweep + Reclaim (Stop Hunt)")
    if liq_bear: score-=35; reasons.append(f"Liquidity Sweep Bearish - EQH {eqh_level:.2f} Sweep + Reject (Stop Hunt)")
    ...
    # 100% CONFIRM LOGIC - Sara signal ek taraf to 100%
    ...
    # Agar 6+ bullish aur 0 bearish to 100% BUY
    if bull_count >=6 and bear_count==0:
        score = 100
    # Agar 6+ bearish aur 0 bullish to 100% SELL
    ...
    # === NEW LOGIC: BUY/SELL/HOLD as per user demand ===
    sl_plot=None; tp1_plot=None; tp2_plot=None
    is_neutral = abs(buy_score - sell_score) <= 2  # 50-50 ya 49-51

    if is_neutral:
        # 50-50 - BUY/SELL box mat dikhao, HOLD dikhao
    ...
    # Save - agar HOLD hai to bhi save karo but SL/TP None
    if sl_plot is None:
