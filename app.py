...
if st.button("START ANALYSIS"):
    ...
    # FIX 3 & 4: FVG Non-Repaint - len-2 + mitigation len-1
    for i in range(len(df)-lookback_fvg, len(df)-2):
        ...
        if h.iloc[i-1] < l.iloc[i+1] and (l.iloc[i+1] - h.iloc[i-1]) > atr_v*0.25:
            ...
            mitigated=False
            for j in range(i+2, len(df)-1): # live candle skip
                if l.iloc[j] <= fvg_low:
        ...
        if l.iloc[i-1] > h.iloc[i+1] and (l.iloc[i-1] - h.iloc[i+1]) > atr_v*0.25:
            fvg_low = float(l.iloc[i+1]); fvg_high = float(h.iloc[i-1])
            mitigated=False
    ...
    # FIX 5: SL/TP Plot Mismatch - Correct Variables
