...
def calculate_analysis(y_sym, tv_sym, pt_mult, pair, tf):
    ...
    reasons=[]; score=50
    if last>e20>e50: score+=4; reasons.append(f"Bullish Trend - Price {last:.5f} > EMA20 {e20:.5f} > EMA50 {e50:.5f}")
    elif last<e20<e50: score-=4; reasons.append(f"Bearish Trend - Price {last:.5f} < EMA20 {e20:.5f} < EMA50 {e50:.5f}")
    if e20>e50>e200 and bos_bull: score+=4; reasons.append(f"BOS Bullish - EMA20>EMA50>EMA200 ({e200:.2f})")
    if e20<e50<e200 and bos_bear: score-=4; reasons.append(f"BOS Bearish - EMA20<EMA50<EMA200")
    ...
    if bull_fvg: score+=15; reasons.append(f"Bullish FVG {bull_fvg[0]:.2f}-{bull_fvg[1]:.2f} ({lookback_fvg} candle me)")
    if bear_fvg: score-=15; reasons.append(f"Bearish FVG {bear_fvg[0]:.2f}-{bear_fvg[1]:.2f} ({lookback_fvg} candle me)")
    if bull_ob: score+=10; reasons.append(f"Bullish Order Block {bull_ob[0]:.2f}-{bull_ob[1]:.2f}")
    if bear_ob: score-=10; reasons.append(f"Bearish Order Block {bear_ob[0]:.2f}-{bear_ob[1]:.2f}")
    if liq_bull: score+=10; reasons.append(f"Bullish Liquidity Sweep - EQL {eql_level:.2f} se reversal")
    if liq_bear: score-=10; reasons.append(f"Bearish Liquidity Sweep - EQH {eqh_level:.2f} se reversal")
    ...
    bull_count = 0
    if last>e20>e50: bull_count+=1
    if bos_bull: bull_count+=1
    if choch_bull: bull_count+=1
    if bull_eng: bull_count+=1
    if bull_fvg: bull_count+=1
    if bull_ob: bull_count+=1
    if liq_bull: bull_count+=1
    bear_count = 0
    if last<e20<e50: bear_count+=1
    if bos_bear: bear_count+=1
    if choch_bear: bear_count+=1
    if bear_eng: bear_count+=1
    if bear_fvg: bear_count+=1
    if bear_ob: bear_count+=1
    if liq_bear: bear_count+=1
...
with cA:
    if st.button("🔔 Turn On Live Notification Tracker"):
        ...
        st.session_state.tracker_y_sym = y_sym
        st.session_state.tracker_tv_sym = tv_sym
        st.session_state.tracker_pt_mult = pt_mult
        st.success(f"Live Tracker ON - {pair} | {tf}")
        st.rerun()
with cB:
    if st.button("🛑 Stop Live Tracker"):
        st.session_state.tracker_on = False
        st.session_state.tracker_pair = None
        st.success("Live Tracker OFF")
        st.rerun()

# === LIVE TRACKER WITH SYSTEM NOTIFICATION ===
if st.session_state.tracker_on and st.session_state.tracker_pair:
    t_pair = st.session_state.tracker_pair
    t_tf = st.session_state.tracker_tf
    t_y = st.session_state.tracker_y_sym
    t_tv = st.session_state.tracker_tv_sym
    t_pt = st.session_state.tracker_pt_mult
    live_result, live_err = calculate_analysis(t_y, t_tv, t_pt, t_pair, t_tf)
    if live_result:
        buy = live_result["buy_score"]; sell = live_result["sell_score"]; mp_live = live_result["mp"]
        atr_v = live_result["atr_v"]
        # === CLEAR LOGIC FOR BUY/SELL % ===
        if abs(buy-sell) <= 2:
            status_text = f"HOLD = {buy}% / {sell}% (No Trade Zone)"
            notify_title = f"HOLD {t_pair}"
            notify_body = f"HOLD {buy}% / SELL {sell}% | Price {mp_live:.2f} | {t_tf}"
            bg_color = "#444400"; border_color = "#FFD700"; emoji="⏸️"
            sl_show = "Wait"
        elif buy > sell:
            if buy >= 75:
                status_text = f"BUY CHANCE = {buy}% | SELL = {sell}% (Strong Buy)"
            elif buy >= 60:
                status_text = f"BUY = {buy}% | SELL = {sell}% (Buy Side)"
            else:
                status_text = f"BUY = {buy}% | SELL = {sell}% (Weak Buy - Score Dropped)"
            notify_title = f"BUY {buy}% {t_pair}"
            notify_body = f"BUY {buy}% | SELL {sell}% | Price {mp_live:.2f} | SL {mp_live - live_result['base_dist']:.2f} | {t_tf}"
            bg_color = "#002200"; border_color = "#00ff88"; emoji="🚀"
            sl_show = f"{mp_live - live_result['base_dist']:.2f}"
        else:
            if sell >= 75:
                status_text = f"SELL CHANCE = {sell}% | BUY = {buy}% (Strong Sell)"
            elif sell >= 60:
                status_text = f"SELL = {sell}% | BUY = {buy}% (Sell Side)"
            else:
                status_text = f"SELL = {sell}% | BUY = {buy}% (Weak Sell - Score Dropped)"
            notify_title = f"SELL {sell}% {t_pair}"
            notify_body = f"SELL {sell}% | BUY {buy}% | Price {mp_live:.2f} | SL {mp_live + live_result['base_dist']:.2f} | {t_tf}"
            bg_color = "#330000"; border_color = "#ff4444"; emoji="📉"
            sl_show = f"{mp_live + live_result['base_dist']:.2f}"

        st.markdown(f"""
        <div style='background:{bg_color}; border:3px solid {border_color}; padding:14px; border-radius:12px; margin-bottom:15px;'>
            <div style='color:#FFD700; font-weight:900; font-size:16px;'>🟨 JANTA AI - LIVE TRACKER</div>
            <div style='color:#fff; font-size:13px; margin-top:6px;'>Pair: {t_pair} | Timeframe: {t_tf}<br>
            Current Status: {emoji} {status_text}<br>
            Live Price: {mp_live:.5f} | SL: {sl_show}</div>
            <div style='color:#aaa; font-size:11px; margin-top:4px;'>Auto refresh 60 sec - Minimize karke bhi chalega | {datetime.now().strftime("%H:%M:%S")}</div>
        </div>
        """, unsafe_allow_html=True)

        # --- SYSTEM NOTIFICATION PUSH - V5 BUTTON FIX - NO PARENT ACCESS ---
        safe_title = notify_title.replace('"','').replace("'","").replace("
"," ")
        safe_body = notify_body.replace('"','').replace("'","").replace("
"," ")
        components.html(f"""
        <div style="font-family:sans-serif;">
            <div id="live-log" style="color:#0f0; font-size:11px; margin-bottom:6px;">System Notif Status: checking...</div>
            <button id="sysBtn" style="background:#FFD700; color:#000; border:none; padding:10px 14px; border-radius:8px; font-weight:900; cursor:pointer; width:100%;">🔔 Enable System Notification (Tap to Allow)</button>
            <div style="color:#aaa; font-size:10px; margin-top:6px;">1st time Allow karna hoga, fir har 60 sec me notification ayega</div>
        </div>
        <script>
        (function(){{
            var title = "{safe_title}";
            var body = "{safe_body}";
            var icon = "https://cdn-icons-png.flaticon.com/512/190/190411.png";
            var logEl = document.getElementById("live-log");
            function updateLog(msg){{ if(logEl) logEl.innerText = msg; console.log(msg); }}

            function canNotify(){{ return ("Notification" in window); }}

            function pushNotifNow() {{
                if (!canNotify()) {{ updateLog("Status: Browser me Notification support nahi"); return; }}
                if (Notification.permission === "granted") {{
                    try {{
                        var n = new Notification(title, {{body: body, icon: icon}});
                        setTimeout(function(){{ try{{n.close()}}catch(e){{}} }}, 10000);
                        updateLog("Status: ✅ Notification bheja - " + new Date().toLocaleTimeString());
                    }} catch(e){{ updateLog("Error: " + e.message); }}
                }} else {{
                    updateLog("Status: Permission = " + Notification.permission + " - Button dabao");
                }}
            }}
            ...
            if (btn) {{
                btn.onclick = function() {{
                    if (!canNotify()) {{ alert("Is browser me Notification support nahi hai"); return; }}
                    updateLog("Permission maang rahe hai...");
                    Notification.requestPermission().then(function(p){{
                        updateLog("Permission result: " + p);
                        if (p === "granted") {{
                            try {{
                                var n = new Notification("JANTA AI Ready ✅", {{body: "System Notification ON - Ab har 60 sec me SELL 61% / BUY 39% ayega", icon: icon}});
                                setTimeout(function(){{ try{{n.close()}}catch(e){{}} }}, 5000);
                                setTimeout(pushNotifNow, 1000);
                            }} catch(e){{}}
                        }} else {{
                            alert("Permission Denied - Chrome > Settings > Site Settings > Notifications > Allow karo");
                        }}
                    }});
                }};
            }}
            ...
            // Auto try on load
            if (canNotify()) {{
                updateLog("Status: Permission=" + Notification.permission);
                if (Notification.permission === "granted") {{
                    // Thoda delay ke baad auto bhejo taki page load ho jaye
                    setTimeout(pushNotifNow, 1200);
                }}
            }} else {{
                updateLog("No Notification API");
            }}
        }})();
        </script>
        """, height=170)
