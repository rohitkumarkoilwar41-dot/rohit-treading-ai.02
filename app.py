...
if st.button("START ANALYSIS"):
    ...
    for i in range(len(df)-4,len(df)-1):
        if h.iloc[i-1]<l.iloc[i+1]: bull_fvg=(h.iloc[i-1],l.iloc[i+1])
        if l.iloc[i-1]>h.iloc[i+1]: bear_fvg=(h.iloc[i+1],l.iloc[i-1])
    bull_ob=None; bear_ob=None
    for i in range(len(df)-15,len(df)-2):
        if c.iloc[i]<o.iloc[i] and c.iloc[i+1]>o.iloc[i+1] and c.iloc[i+2]>h.iloc[i]: bull_ob=(l.iloc[i],h.iloc[i])
        if c.iloc[i]>o.iloc[i] and c.iloc[i+1]<o.iloc[i+1] and c.iloc[i+2]<l.iloc[i]: bear_ob=(l.iloc[i],h.iloc[i])
    ...
    if bull_fvg: score+=10; reasons.append(f"Bullish FVG {bull_fvg[0]:.2f}-{bull_fvg[1]:.2f}")
    if bear_fvg: score-=10; reasons.append(f"Bearish FVG {bear_fvg[0]:.2f}-{bear_fvg[1]:.2f}")
    ...
    if bull_fvg: fig.add_hrect(y0=bull_fvg[0],y1=bull_fvg[1],fillcolor="green",opacity=0.25,annotation_text="Bull FVG")
    if bear_fvg: fig.add_hrect(y0=bear_fvg[0],y1=bear_fvg[1],fillcolor="red",opacity=0.25,annotation_text="Bear FVG")
