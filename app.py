...
if st.button("START ANALYSIS"):
    ...
    for i in range(len(df)-lookback_fvg, len(df)-1):
        if h.iloc[i-1] < l.iloc[i+1] and (l.iloc[i+1] - h.iloc[i-1]) > atr_v*0.25:
            fv=(float(h.iloc[i-1]), float(l.iloc[i+1])); bull_fvg_list.append(fv); bull_fvg=fv
        if l.iloc[i-1] > h.iloc[i+1] and (l.iloc[i-1] - h.iloc[i+1]) > atr_v*0.25:
            fv=(float(h.iloc[i+1]), float(l.iloc[i-1])); bear_fvg_list.append(fv); bear_fvg=fv
    ...
    for fv in bull_fvg_list[-3:]: fig.add_hrect(y0=fv[0], y1=fv[1], fillcolor="green", opacity=0.25, line_width=0)
    for fv in bear_fvg_list[-3:]: fig.add_hrect(y0=fv[0], y1=fv[1], fillcolor="red", opacity=0.25, line_width=0)
