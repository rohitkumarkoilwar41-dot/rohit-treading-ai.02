...
.live-tracker-fixed{position:fixed; top:0; left:0; right:0; z-index:999999; background:#111; border-bottom:3px solid #FFD700; padding:10px; box-shadow:0 4px 15px rgba(0,0,0,0.8);}
.floating-mini{position:fixed; bottom:20px; right:20px; z-index:999999; width:320px; background:#000; border:2px solid #FFD700; border-radius:12px; padding:10px;}
</style>
...
# --- SESSION STATE FOR LIVE TRACKER ---
...
# === BUTTON LAYOUT - PURANA BUTTON SAFE ===
...
# === LIVE TRACKER BUTTONS - NAYA ADDITION (MINIMIZE WALA) ===
cA,cB = st.columns(2)
with cA:
    if st.button("🔔 Turn On Live Notification Tracker"):
        ...
        st.success(f"Live Tracker ON - {pair} | {tf} - Har 1 min me auto update hoga")
        st.rerun()
with cB:
    if st.button("🛑 Stop Live Tracker"):
        st.session_state.tracker_on = False
        st.session_state.tracker_pair = None
        st.success("Live Tracker OFF - Notification gayab")
        st.rerun()

# === LIVE TRACKER DISPLAY (STICKY TOP + FLOATING MINI) ===
if st.session_state.tracker_on and st.session_state.tracker_pair:
    ...
    if live_result:
        ...
        # Status logic
        if abs(buy-sell) <=2:
            status_html = f"⏸️ HOLD = {buy}% (No Trade Zone)"
            bg_color = "#444400"
            border_color = "#FFD700"
            emoji = "⏸️"
            sl_show = "None"
            ...
            if buy >= 75:
                status_html = f"🚀 BUY CHANCE = {buy}% (Order Block Active)" if live_result["bull_ob"] else f"🚀 BUY CHANCE = {buy}%"
            else:
                status_html = f"⚠️ WARNING = {buy}% (Score Dropped!)" if buy<75 and buy>60 else f"🚀 BUY = {buy}%"
            bg_color = "#002200"
            border_color = "#00ff88"
            emoji = "🚀"
            sl_calc = mp_live - live_result["base_dist"]
            sl_show = f"{sl_calc:.2f}"
        else:
            if sell >= 75:
                status_html = f"📉 SELL CHANCE = {sell}% (Liquidity Swept)" if live_result["liq_bear"] else f"📉 SELL CHANCE = {sell}%"
            else:
                status_html = f"⚠️ WARNING = {buy}% (Score Dropped!)" if buy<75 else f"📉 SELL = {sell}%"
            bg_color = "#330000"
            border_color = "#ff4444"
            emoji = "📉"
            sl_calc = mp_live + live_result["base_dist"]
        ...
        # STICKY LIVE TRACKER BOX - UPAR FIXED
        st.markdown(f"""
        <div style='background:{bg_color}; border:3px solid {border_color}; padding:14px; border-radius:12px; margin-bottom:15px;'>
            ...
            Current Status: {status_html}<br>
            Live Price: {mp_live:.5f} | SL: {sl_show}</div>
            <div style='color:#aaa; font-size:11px; margin-top:4px;'>Auto refresh har 60 sec - Minimize karke bhi chalega | {datetime.now().strftime("%H:%M:%S")}</div>
        </div>
        """, unsafe_allow_html=True)

        # FLOATING MINI VIEW - Picture in Picture jaisa
        components.html(f"""
        <div id="floatBox" style="position:fixed; bottom:20px; right:20px; width:300px; background:#000; border:2px solid {border_color}; border-radius:14px; padding:12px; z-index:9999; box-shadow:0 0 15px {border_color}; font-family:sans-serif;">
            <div style="color:#FFD700; font-weight:900; font-size:12px;">🟨 LIVE TRACKER - {t_pair}</div>
            <div style="color:#fff; font-size:11px; margin-top:4px;">{t_tf} | {status_html}<br>Price: {mp_live:.2f} | SL: {sl_show}</div>
            <div style="color:#0f0; font-size:10px; margin-top:4px;">● LIVE - {datetime.now().strftime("%H:%M:%S")}</div>
        </div>
        """, height=0)

# === PURANA ANALYSIS DISPLAY (if START pressed) ===
if "last_result" in st.session_state and st.session_state.last_result:
    ...
    y_sym=result["y_sym"]; tv_sym=result["tv_sym"]; pt_mult=PAIRS[pair][2] if pair in PAIRS else 100
...
# Auto Refresh for Live Tracker - Har 60 sec
