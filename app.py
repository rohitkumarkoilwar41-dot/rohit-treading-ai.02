...
if st.session_state.tracker_on and st.session_state.tracker_pair:
    ...
    if live_result:
        ...
        # --- SYSTEM NOTIFICATION V7 - FIXED BUTTON - NO PARENT ---
        safe_title = str(notify_title).replace('"', '').replace("'", '')
        safe_body = str(notify_body).replace('"', '').replace("'", '')
        html_template = """
        <div style="font-family:sans-serif;">
            <div id="live-log" style="color:#0f0; font-size:11px; margin-bottom:6px;">Checking permission...</div>
            ...
            <div style="color:#aaa; font-size:10px; margin-top:6px;">1st time Allow karo, fir har 60 sec notification ayega</div>
        </div>
        <script>
        (function(){
            var title = "TITLE_HERE";
            var body = "BODY_HERE";
            var icon = "https://cdn-icons-png.flaticon.com/512/190/190411.png";
            var logEl = document.getElementById("live-log");
            function log(m){ if(logEl) logEl.innerText=m; }
            function canNotify(){ return ("Notification" in window); }
            function pushNow(){
                if(!canNotify()){ log("No support"); return; }
                if(Notification.permission==="granted"){
                    try{
                        var n=new Notification(title,{body:body,icon:icon});
                        setTimeout(function(){try{n.close()}catch(e){}},10000);
                        log("Sent at "+new Date().toLocaleTimeString());
                    }catch(e){ log(e.message); }
                } else {
                    log("Permission="+Notification.permission+" - Tap button");
                }
            }
            var btn=document.getElementById("sysBtn");
            if(btn){
                btn.onclick=function(){
                    if(!canNotify()){ alert("No support"); return; }
                    log("Requesting...");
                    Notification.requestPermission().then(function(p){
                        log("Result="+p);
                        if(p==="granted"){
                            new Notification("JANTA AI Ready",{body:"ON - har 60 sec ayega",icon:icon});
                            setTimeout(pushNow,1000);
                        } else {
                            alert("Denied - Chrome Settings > Notifications > Allow");
                        }
                    });
                };
            }
            log("Permission="+(canNotify()?Notification.permission:"no api"));
            if(canNotify() && Notification.permission==="granted"){ setTimeout(pushNow,1200); }
        })();
        </script>
        """
        html_code = html_template.replace("TITLE_HERE", safe_title).replace("BODY_HERE", safe_body)
