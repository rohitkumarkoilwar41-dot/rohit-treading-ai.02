...
if st.session_state.tracker_on and st.session_state.tracker_pair:
    ...
    if live_result:
        ...
        safe_title = notify_title.replace('"','').replace("'","")
        safe_body = notify_body.replace('"','').replace("'","")
        components.html(f"""
        <script>
        function sendNotif() {{
            if (!("Notification" in window)) {{ console.log("No notif support"); return; }}
            if (Notification.permission === "granted") {{
                var n = new Notification("{safe_title}", {{ body: "{safe_body}", icon: "https://cdn-icons-png.flaticon.com/512/190/190411.png" }});
                setTimeout(() => n.close(), 8000);
            }} else if (Notification.permission !== "denied") {{
                Notification.requestPermission().then(function(p) {{
                    if (p === "granted") {{
                        var n = new Notification("{safe_title}", {{ body: "{safe_body}" }});
                        setTimeout(() => n.close(), 8000);
                    }}
                }});
            }}
