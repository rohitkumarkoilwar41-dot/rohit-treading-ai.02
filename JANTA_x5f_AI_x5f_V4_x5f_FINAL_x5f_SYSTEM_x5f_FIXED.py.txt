...
if st.session_state.tracker_on and st.session_state.tracker_pair:
    ...
    if live_result:
        ...
        # --- SYSTEM NOTIFICATION PUSH - FINAL FIX FOR ANDROID SHADE ---
        safe_title = notify_title.replace('"','').replace("'","").replace("\n"," ")
        safe_body = notify_body.replace('"','').replace("'","").replace("\n"," ")
        # This HTML tries parent window notification + iframe notification + permission button
        components.html(f"""
        ...
        (function(){{
            ...
            function pushNotif(){{
                ...
                if (API.permission === "granted") {{
                    try {{
                        var n = new API(title, {{body: body, icon: icon, requireInteraction: false}});
                        setTimeout(function(){{ try{{n.close()}}catch(e){{}} }}, 10000);
                    }} catch(e){{ console.log("Notif error", e); }}
                }} else if (API.permission !== "denied") {{
                    API.requestPermission().then(function(p){{
                        if (p === "granted") {{
                            try {{
                                var n = new API(title, {{body: body, icon: icon}});
                                setTimeout(function(){{ try{{n.close()}}catch(e){{}} }}, 10000);
                            }} catch(e){{}}
                        }}
                    }});
            ...
            // User gesture ke baad hi allowed hai, isliye Turn On ke click ke turant baad call
            pushNotif();
            // Auto refresh ke liye bhi 1 sec baad fir se try (permission milne ke baad)
            setTimeout(pushNotif, 1500);
        }})();
        </script>
        <div style="margin-top:8px; font-family:sans-serif;">
            <button onclick="(function(){{
                var API = (window.parent && window.parent.Notification) ? window.parent.Notification : window.Notification;
                if(!API){{ alert('Is browser me Notification support nahi hai'); return; }}
                API.requestPermission().then(function(p){{
                    if(p==='granted'){{
                        new API('JANTA AI Ready', {{body: 'System Notification ON - Ab har 60 sec me upar notification ayega'}});
                    }} else {{
                        alert('Permission denied - Chrome Settings > Site Settings > Notifications > Allow karo');
                    }}
                }});
            }})()" style="background:#FFD700; color:#000; border:none; padding:8px 12px; border-radius:8px; font-weight:900; cursor:pointer;">🔔 Enable System Notification (1st time only)</button>
            <div style="color:#aaa; font-size:10px; margin-top:4px;">Agar upar shade me nahi aa raha to ye button dabao > Allow karo</div>
        </div>
        """, height=120)
