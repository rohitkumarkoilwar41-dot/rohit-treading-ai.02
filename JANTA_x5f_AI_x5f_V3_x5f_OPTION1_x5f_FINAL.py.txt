...
def calculate_analysis(y_sym, tv_sym, pt_mult, pair, tf):
    ...
    # Support Resistance - 30 candle ka
    ...
    # === FVG - FIXED, Mitigation check ke saath ===
    ...
    # === ORDER BLOCK - 15 candle wala original ===
    ...
    # === FIXED 100% SCORING - OPTION 1: PURANE JAISA HI BUY+SELL=100 ===
    # Base 50 se start, purane jaisa
    score = 50
    reasons=[]
    buy_pts=0; sell_pts=0

    # EMA 4 point
    if last>e20>e50: score+=4; buy_pts+=4; reasons.append(f"✅ EMA Bullish +4 - Price {last:.5f} > EMA20 {e20:.5f} > EMA50 {e50:.5f}")
    elif last<e20<e50: score-=4; sell_pts+=4; reasons.append(f"✅ EMA Bearish +4 - Price {last:.5f} < EMA20 {e20:.5f} < EMA50 {e50:.5f}")
    else: reasons.append(f"⚪ EMA Neutral 0")

    # BOS 4 point
    if bos_bull: score+=4; buy_pts+=4; reasons.append(f"✅ BOS Bullish +4 - EMA20>EMA50>EMA200 ({e200:.2f})")
    elif bos_bear: score-=4; sell_pts+=4; reasons.append(f"✅ BOS Bearish +4 - EMA20<EMA50<EMA200")
    else: reasons.append(f"⚪ BOS Neutral 0")

    # CHoCH 10 point
    if choch_bull: score+=10; buy_pts+=10; reasons.append(f"✅ CHoCH Bullish +10 - Swing High Break {recent_high:.2f}")
    if choch_bear: score-=10; sell_pts+=10; reasons.append(f"✅ CHoCH Bearish +10 - Swing Low Break {recent_low:.2f}")

    # Engulfing 10 point
    if bull_eng: score+=10; buy_pts+=10; reasons.append("✅ Bullish Engulfing +10 - Strong Buy Signal")
    if bear_eng: score-=10; sell_pts+=10; reasons.append("✅ Bearish Engulfing +10 - Strong Sell Signal")

    # Liquidity Sweep 30 KING - EQH/EQL
    if liq_bull: score+=30; buy_pts+=30; reasons.append(f"🔥 Bullish Liquidity Sweep +30 KING - EQL {eql_level:.2f} se reversal (Stop-Hunt)")
    if liq_bear: score-=30; sell_pts+=30; reasons.append(f"🔥 Bearish Liquidity Sweep +30 KING - EQH {eqh_level:.2f} se reversal (Stop-Hunt)")

    # Fibonacci 16
    if fib_bull: score+=16; buy_pts+=16; reasons.append(f"✅ Fibonacci Golden Zone +16 - Golden Pocket {fib_level:.2f} me (Discount Zone)")
    if fib_bear: score-=16; sell_pts+=16; reasons.append(f"✅ Fibonacci Premium Zone +16 - Premium {fib_level:.2f} me")

    # Order Block + FVG combined as 16 point (per photo)
    if bull_ob: score+=16; buy_pts+=16; reasons.append(f"✅ Bullish Order Block +16 - {bull_ob[0]:.2f}-{bull_ob[1]:.2f} (1H OB)")
    if bull_fvg and not bull_ob: score+=16; buy_pts+=16; reasons.append(f"✅ Bullish FVG +16 - {bull_fvg[0]:.2f}-{bull_fvg[1]:.2f}")
    if bear_ob: score-=16; sell_pts+=16; reasons.append(f"✅ Bearish Order Block +16 - {bear_ob[0]:.2f}-{bear_ob[1]:.2f}")
    if bear_fvg and not bear_ob: score-=16; sell_pts+=16; reasons.append(f"✅ Bearish FVG +16 - {bear_fvg[0]:.2f}-{bear_fvg[1]:.2f}")

    # POC 10
    if poc_bull: score+=10; buy_pts+=10; reasons.append(f"✅ Volume POC Support +10 - POC Level {poc_level:.2f} se bounce")
    if poc_bear: score-=10; sell_pts+=10; reasons.append(f"✅ Volume POC Resistance +10 - POC Level {poc_level:.2f} se rejection")

    # Jackpot note only, no forced HOLD
    is_jackpot_buy = (liq_bull and fib_bull and (bull_ob or bull_fvg))
    is_jackpot_sell = (liq_bear and fib_bear and (bear_ob or bear_fvg))
    if is_jackpot_buy:
        reasons.append(f"💎 JACKPOT BUY Combo - 30+16+16+10 = {buy_pts}% (Bottom Price)")
    if is_jackpot_sell:
        reasons.append(f"💎 JACKPOT SELL Combo - 30+16+16+10 = {sell_pts}% (Top Price)")

    reasons.append(f"LIVE {last:.5f} = {tv_sym} {last:.5f} | ATR {atr_v:.2f} | Support {supp:.2f} | Resistance {res:.2f}")
    ...
    # === DELETED: bull_count >=6 bypass filter ===
...
if st.session_state.tracker_on:
    ...
    if result_t:
        ...
        if buy_s > sell_s:
            tg_status = f"BUY {buy_s}%"
            tg_emoji = "🚀"
        elif sell_s > buy_s:
            tg_status = f"SELL {sell_s}%"
            tg_emoji = "📉"
        else:
            tg_status = f"HOLD BUY {buy_s}% SELL {sell_s}%"
            tg_emoji = "⏸️"
...
if "last_result" in st.session_state and st.session_state.last_result:
    ...
    with st.expander("🔍 Fixed 100% Model + Support/Resistance Check"):
        st.write(f"✅ EMA 20 & 50 Trend: 4 point - Working")
        st.write(f"✅ BOS Break of Structure: 4 point - Working (EMA200 check)")
        st.write(f"✅ CHoCH: 10 point - Working (Recent High/Low Break)")
        st.write(f"✅ Engulfing: 10 point - Working")
        st.write(f"✅ Liquidity Sweep KING EQH/EQL: 30 point - Fixed Latest Wala - Working")
        st.write(f"✅ Fibonacci Golden Zone 0.5-0.618: 16 point - Working")
        st.write(f"✅ Order Block (15 candle): 16 point - Working")
        st.write(f"✅ FVG Fair Value Gap (Mitigation check): 16 point - Working (OB nahi toh FVG)")
        st.write(f"✅ Volume POC: 10 point - Working")
        st.write(f"✅ Support: {supp:.2f} (Last 30 Low) - Working")
        st.write(f"✅ Resistance: {res:.2f} (Last 30 High) - Working")
        st.write(f"✅ RSI, ATR - Working")
        st.write(f"✅ Telegram Tracker - Working")
        st.write(f"Total Fixed = 4+4+10+10+30+16+16+10 = 100%")
        st.write(f"Current Score = 50 + BUY({result.get('buy_pts',0)}) - SELL({result.get('sell_pts',0)}) = {buy_score}% BUY / {sell_score}% SELL")
    ...
    fig.add_hline(y=supp, line_color="#00ff88", line_dash="dot", annotation_text=f"Support {supp:.2f}")
    fig.add_hline(y=res, line_color="#ff4444", line_dash="dot", annotation_text=f"Resistance {res:.2f}")
